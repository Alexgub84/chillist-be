 ...
 * Run manually:  npm run test:ai-suggestions-stream-e2e
 *
 * Requires .env with AI_PROVIDER and the matching API key.
 * Result file → logs/ai-suggestions-stream-e2e-<ts>.json
 */
...
describe('AI Suggestions Stream E2E — Real AI Model', () => {
  ...
  it('streams valid SSE events with correct item structure per category', async () => {
    ...
    for (const ev of suggestionEvents) {
      ...
      const suggestions = ev.data.suggestions as Array<Record<string, unknown>>
      for (const item of suggestions) {
        expect(item.category).toBe(category)
        expect(ITEM_CATEGORY_VALUES as readonly string[]).toContain(
          item.category as string
        )
        expect(UNIT_VALUES as readonly string[]).toContain(item.unit as string)
