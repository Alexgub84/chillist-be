/**
 * Prompt quality — real AI model.
 *
 * Isolated in tests/ai-quality/ — never runs during `npm run test`.
 * Run manually:  npm run test:ai-prompt-quality
 *
 * Requires .env with AI_PROVIDER and the matching API key.
 * Tests whether the AI understood the context, not schema compliance (Zod handles that).
 */
...
describe('Prompt quality — heuristic assertions (real API)', () => {
  // -------------------------------------------------------------------------
  ...
  describe('Context sensitivity', () => {
    ...
    it('winter camping includes warm clothing / cold weather gear', async () => {
      const { suggestions } = await runScenario('winter', SCENARIOS.winter.plan)
  ...
  describe('Hebrew output (camping) — names/reasons in Hebrew', () => {
    it('generates 15–50 items with Hebrew script in most item names or reasons', async () => {
      ...
      expect(withHebrew.length / suggestions.length).toBeGreaterThanOrEqual(0.6)
    }, 60_000)
